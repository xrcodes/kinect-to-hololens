...
public:
    VideoRenderer(const int receiver_id, const asio::ip::udp::endpoint remote_endpoint, int width, int height)
        : width_{width}, height_{height}, color_decoder_{}, depth_decoder_{width * height}, last_frame_id_{std::nullopt}, last_frame_time_{tt::TimePoint::now()}
    {
    }

    std::optional<int> last_frame_id() { return last_frame_id_; }
