...
Samples::PointCloudGenerator::PointCloudGenerator(const k4a_calibration_t& sensorCalibration)
{
    ...
    if(k4a_image_create(K4A_IMAGE_FORMAT_CUSTOM,
                        ...
                        depthHeight,
                        depthWidth * (int)sizeof(PointCloudPixel_int16x3_t),
                        &m_pointCloudImage_int16x3) != K4A_RESULT_SUCCEEDED)
        throw std::runtime_error("Create Point Cloud Image failed!");
}
...
void Samples::PointCloudGenerator::Update(k4a_image_t depthImage)
{
    ...
    if(k4a_transformation_depth_image_to_point_cloud(m_transformationHandle,
                                                     depthImage,
                                                     K4A_CALIBRATION_TYPE_DEPTH,
                                                     m_pointCloudImage_int16x3) != K4A_RESULT_SUCCEEDED)
        throw std::runtime_error("Transform depth image to point clouds failed!");
}
