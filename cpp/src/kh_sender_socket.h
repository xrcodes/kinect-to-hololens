...
public:
    ...
    void sendAudioPacket(int session_id, int frame_id, const std::vector<uint8_t>& opus_frame, int opus_frame_size, std::error_code& error);
    std::optional<std::vector<std::byte>> receive(std::error_code& error);
    static std::vector<std::byte> createFrameMessage(float frame_time_stamp, bool keyframe, const std::vector<std::byte>& vp8_frame,
